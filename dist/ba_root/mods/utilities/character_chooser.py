...
def __init__(self, vpos: float, sessionplayer: bs.SessionPlayer, lobby: Lobby) -> None:
    ...
    self._random_color, self._random_highlight = bs.get_player_profile_colors(None)
...
def _set_ready(self, ready: bool) -> None:
    ...
    if not ready:
        ...
        self._sessionplayer.assigninput(
            (babase.InputType.UP_PRESS),
            babase.CallStrict(
                self.handlemessage, ChangeMessage("characterchooser", -1)
            ),
        )
        self._sessionplayer.assigninput(
            (babase.InputType.DOWN_PRESS),
        ...
        device_profiles = _babase.app.config.setdefault("Default Player Profiles", {})
